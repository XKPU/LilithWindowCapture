    ...
    internal static class TrayIntegration
    {
        ...
        private static void Toggle()
        {
            try
            {
                ...
                _log?.LogInfo($"[LilithWindowCapture] 切换窗口录制模式 -> {(next ? "开启" : "关闭")}（已写入配置）");
                try
                {
                    ShowSystemTray.instance?.tray?.ShowNotification(
                        "LilithWindowCapture", next ? "已切换至窗口录制模式" : "已关闭窗口录制模式", 3000);
                }
                catch { /* 通知失败不影响切换 */ }
            }
            catch (Exception e)
        ...
        public static void TryRegister()
        {
            try
            {
                ...
                _registeredCount = 1;
                _initialized = true;
                _log?.LogInfo("[LilithWindowCapture] 系统托盘菜单已注册窗口录制开关（当前：" +
